...
positpal::start("Groupe_4_CC2.Rmd")
```
...
1. **Apolline Bonneville** — 20629019
    + [BIOL343_CC](https://github.com/apo1245/BIOL343_CC)
1. 
...
-----------|-------------|--------
population_type| asexual or sexual individue | no unit
ovary_width | width of the ovary | mm
ovary_height | height of the ovary | mm
flower_number | number of flower | no unit
...
```{r import-clean}
cc1_data_no <- read.csv("A1_Data_Excel.csv")
cc1_data_no <- cc1_data_no[1:46,]
cc1_data <- cc1_data_no[ , c('population_type', 'ovary_width', 'ovary_height', 'flower_number')]
colnames(cc1_data) <- c("Population_Type", "Ovary_Width", "Ovary_Height", "Flower_Number")
...
$$ \text{Ovary with} \times \text{Ovary area} = \text{ovary area} $$
Ovary area is measure in $\text{mm}^2$
...
```{r area-table, echo=F}
library(knitr)
kable(area_means, caption = "area mean by population type")
```
...
library(ggplot2)
Plot <- ggplot(aes(ovary_area), data = cc1_data)+ 
    geom_histogram(binwidth=1) # or binwidth=1

Plot + facet_wrap(.~Population_Type)
```
...
![Solanum pimpinellifolium](https://alchetron.com/cdn/solanum-pimpinellifolium-f5becb15-ce78-449b-b63d-eba53af4cef-resize-750.jpeg)

Source: https://alchetron.com/Solanum-pimpinellifolium
